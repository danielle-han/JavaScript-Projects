...
function Input_Digit(digit) {
    ...
    if (Wait_Second_Operand === true) {
        ...
        //this overwrites DisplayValue if current value is 0
...
function Input_Decimal(dot) {
    //this ensures that accidental clicking of decimal point doesnt cause bugs in operation
...
function Handle_Operator(Next_Operator) {
    ...
    //when an operator key is pressed we convert the current number displayed on screen to a number and then store the result in 
    ...
    if (First_Operand == null) {
        ...
        //if exists, property lookup is performed for the operator in perform_calculation object and function that matches the operator is executed
...
keys.addEventListener('click', (event) => {
    //the target var is an object that represents and element that was clicked
    const { target } = event;
